...
# کیفیت/منطق Gemini عمداً تغییری نکرده است.
MAX_ITEMS_PER_RUN = 5

# فاصله بین پیام‌های تلگرام
SEND_DELAY_SECONDS = 1.0

# Gemini sometimes returns transient 429/5xx/timeouts.
# Google recommends exponential backoff + jitter for these errors.
# 5 total attempts = initial request + 4 retries.
...
# Telegram can also have transient network/API errors.
TELEGRAM_MAX_ATTEMPTS = 4
TELEGRAM_BASE_DELAY = 2.0
TELEGRAM_MAX_DELAY = 20.0

# حداکثر طول پیام تلگرام
TELEGRAM_MAX_LEN = 4096

# timeout برای دریافت RSS
HTTP_TIMEOUT = 20
...
def collect_today_entries(state: dict) -> list[dict]:
    ...
    session.headers.update({
        "User-Agent": "Kran-Foolad-News-Bot/3.0 (+https://github.com/)"
    })

    for feed_url in RSS_FEEDS:
        try:
            # به‌جای feedparser.parse(URL)، RSS را با timeout مشخص می‌گیریم
            # تا یک منبع خراب نتواند اجرای GitHub Actions را معطل کند.
...
def _retry_after_seconds(response) -> Optional[float]:
    """Retry-After را در صورت وجود، با محدودیت امن برمی‌گرداند."""
    try:
        value = response.headers.get("Retry-After") if response is not None else None
        if not value:
            return None
        seconds = float(value)
        return max(0.0, min(seconds, GEMINI_MAX_DELAY))
    except (TypeError, ValueError):
        return None


def _gemini_backoff(attempt_index: int, retry_after: Optional[float] = None) -> float:
    """Exponential backoff + jitter مطابق توصیه Gemini API."""
    if retry_after is not None:
        base = retry_after
    else:
        base = min(GEMINI_MAX_DELAY, GEMINI_BASE_DELAY * (2 ** attempt_index))
    return base + random.uniform(0, GEMINI_JITTER)


def _validate_gemini_result(result: object) -> dict:
    ...
    if missing:
        raise ValueError(f"Gemini JSON is missing required fields: {missing}")

    allowed_categories = {
        "بازار و قیمت", "تولید", "صادرات و واردات", "بورس کالا",
        "قوانین و سیاست‌گذاری", "شرکت‌ها و کارخانه‌ها", "مواد اولیه",
        "بازار جهانی", "سایر",
    }
    if result["news_type"] not in {"خبر", "غیرخبری"}:
        raise ValueError("Invalid news_type from Gemini.")
    if result["category"] not in allowed_categories:
        raise ValueError("Invalid category from Gemini.")
    if result["importance"] not in {"مهم", "عادی"}:
        raise ValueError("Invalid importance from Gemini.")
    if result["market_impact"] not in {"افزایشی", "کاهشی", "نامشخص"}:
        raise ValueError("Invalid market_impact from Gemini.")
    if not isinstance(result["relevant"], bool):
        raise ValueError("Invalid relevant value from Gemini.")

    for key in ("title", "summary", "analysis"):
        if not isinstance(result[key], str) or not result[key].strip():
            raise ValueError(f"Gemini returned an empty/invalid {key}.")
...
def analyze_with_gemini(entry: dict) -> Optional[dict]:
    """
    Gemini را با retry مقاوم اجرا می‌کند.

    نکته مهم: 503/429/5xx/408 و timeout خطای موقتی محسوب می‌شوند.
    خطاهای 400/401/403 و پاسخ‌های نامعتبر به‌عنوان خطای دائمی همین خبر
    تلقی می‌شوند تا ربات وارد retry بی‌نهایت نشود.
    """
        ...
        try:
            ...
            if status in transient_statuses:
                ...
                raise requests.exceptions.HTTPError(
                    f"HTTP {status}: {error_preview}",
                    response=response,
                )

            # 4xxهای غیرموقت را بی‌جهت تکرار نمی‌کنیم.
            response.raise_for_status()

            data = response.json()
            candidates = data.get("candidates") or []
            if not candidates:
                raise ValueError(f"Gemini returned no candidates: {str(data)[:500]}")
            ...
            return _validate_gemini_result(result)
            ...
            delay = _gemini_backoff(attempt, retry_after)
            print(
                f"[warn] Gemini temporary HTTP {status} for '{entry['title']}' "
            ...
            delay = _gemini_backoff(attempt)
            print(
                f"[warn] Gemini network/timeout failure for '{entry['title']}' "
                f"(attempt {attempt + 1}/{GEMINI_MAX_ATTEMPTS}); "
            ...
            # پاسخ malformed معمولاً transient نیست؛ فقط یک بار دیگر تلاش می‌کنیم
            # چون ممکن است پاسخ شبکه‌ای ناقص یا خطای لحظه‌ای در تولید JSON باشد.
            last_error = exc
            if attempt >= 1:
                break
            ...
            delay = _gemini_backoff(attempt)
            print(
                f"[warn] Gemini response validation failure for '{entry['title']}' "
                f"(attempt {attempt + 1}/{GEMINI_MAX_ATTEMPTS}); "
                f"retrying in {delay:.1f}s: {exc}",
                file=sys.stderr,
            )
    ...
    print(
        f"[warn] Gemini failed after {GEMINI_MAX_ATTEMPTS} attempts for "
        f"'{entry['title']}': {last_error}",
        file=sys.stderr,
    )
    return None
...
def telegram_request(method: str, payload: dict) -> dict:
        ...
        try:
            ...
            if status in transient_statuses:
                ...
                if retry_after is None:
                    retry_after = min(
                        TELEGRAM_MAX_DELAY,
                        TELEGRAM_BASE_DELAY * (2 ** attempt),
                    ) + random.uniform(0, 2)
                else:
                    retry_after = min(max(retry_after, 0), TELEGRAM_MAX_DELAY)

                last_error = RuntimeError(
                    f"Telegram HTTP {status}: {response.text[:500]}"
                )
                if attempt < TELEGRAM_MAX_ATTEMPTS - 1:
                    print(
                        f"[warn] Telegram temporary HTTP {status}; "
            ...
            if attempt < TELEGRAM_MAX_ATTEMPTS - 1:
                ...
                print(
                    f"[warn] Telegram network/timeout failure; "
                    f"retrying in {delay:.1f}s: {exc}",
                    file=sys.stderr,
                )
                time.sleep(delay)
    ...
    raise RuntimeError(f"Telegram request failed after {TELEGRAM_MAX_ATTEMPTS} attempts: {last_error}")
...
def main() -> None:
    ...
    print(f"[info] Gemini max attempts: {GEMINI_MAX_ATTEMPTS}, timeout: {GEMINI_TIMEOUT}s")
    print(f"[info] max candidates this run: {MAX_ITEMS_PER_RUN}")
